...
def wiki_search():
    global recognizer
    speaker.say("What do you want to know about")
    speaker.runAndWait()
    done =False
    while not done:
        try:
        ...
        try:
            with speech_recognition.Microphone() as mic:
                ...
                if 'close'.casefold() in query or 'exit' in query or 'quit' in query or 'leave' in query :
                    ...
                    google_search()
                elif 'what is'.casefold() in query or 'wikipedia'.casefold() in query:
                    wiki_search()
                else:
                    speaker.say('Sorry Sir! These are beyond my Limitaions.')
                    speaker.runAndWait()
        except speech_recognition.UnknownValueError:
            recognizer=speech_recognition.Recognizer()
            speaker.runAndWait()
